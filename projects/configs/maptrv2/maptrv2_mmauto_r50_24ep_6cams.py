...
model = dict(
    ...
    pts_bbox_head=dict(
        type='MapTRv2Head', # plugin/maptr/densheads/maptrv2_head.py 上面两步得到了特征 [1,6,256,15,25]
        bev_h=bev_h_,
        bev_w=bev_w_,
        num_query=900,
        num_vec_one2one=50,
        ...
        # z_cfg=z_cfg,
        transformer=dict(
            type='MapTRPerceptionTransformer', # plugin/maptr/modules/transformer.py
            ...
            encoder=dict(
                type='LSSTransform', # plugin/maptr/modules/encoder.py, 相机内参等在这里使用
                in_channels=_dim_,
                out_channels=_dim_,
                feat_down_sample=32,
...
dataset_type = 'MMautoDataset_6cams' 
data_root = 'data/MMAutoDataLog-2025-04-03-16-27-32/'
...
data = dict(
    ...
    test=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=data_root + 'camera_imx490_group_intrinsics.json',
